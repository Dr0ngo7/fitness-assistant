...
const SLUGS = {
  Chest:'gogus', Back:'sirt', Shoulders:'omuz', Arms:'kol', Legs:'bacak', Core:'karin',
};
...
export default function MuscleMap() {
  ...
  const onPick = (label) => {
    ...
    setSelected(label);
    if (!slug) { Alert.alert('Eşleşme yok', `"${label}" için slug tanımlı değil.`); return; }
    router.push({ pathname: '/exercises/[group]', params: { group: slug } });
  };
    ...
    <SafeAreaView style={{ flex:1, backgroundColor:'#eef2f7' }} edges={['top']}>
      <View style={{ padding: 16, paddingBottom: 8 }}>
        <View style={{ flexDirection:'row', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <TouchableOpacity onPress={() => router.push('/exercises/all')}
        ...
        <View style={{ flexDirection:'row', gap:8, marginTop:12 }}>
          {['front','back'].map((s)=>(
            <TouchableOpacity key={s} onPress={()=>setSide(s)}
      ...
      <View style={{ marginHorizontal:12, borderRadius:16, overflow:'hidden', backgroundColor:'#fff' }}>
        <ImageBackground source={imgSrc} resizeMode="contain"
          style={{ width:'100%', aspectRatio:3/4, position:'relative' }}>
          {(AREAS||[]).map((a,i)=>(
            <TouchableOpacity key={`${side}-${i}`} activeOpacity={0.65}
              onPress={()=>onPick(a.key)}
              style={[{ position:'absolute', backgroundColor: selected===a.key?'rgba(14,165,233,0.28)':'rgba(14,165,233,0.12)',
                        borderRadius:14, borderWidth:1, borderColor:'rgba(14,165,233,0.35)' }, a.style]}
            />
          ))}
        </ImageBackground>
      </View>
      ...
      <Text style={{ opacity:0.7, paddingHorizontal:16, paddingTop:12, paddingBottom:8 }}>
        Bölgeye dokun → ilgili egzersizler.
      </Text>
    </SafeAreaView>
  );
}
